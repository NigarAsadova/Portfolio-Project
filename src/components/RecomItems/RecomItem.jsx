...
const RecomItem = ({ Recommendations }) => {
  ...
  return (
      <Container sx={{ display: 'flex', justifyContent: 'center', padding: '0 20px 20px 20px' }} >
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: '15px', justifyContent: 'center'}} maxWidth="md" >
                {Recommendations.map((recom, index) => (
                    ...
                    >
                      ...
                      <Typography  sx={{ color: 'rgba(118, 118, 118, 1)', fontSize:'14px', fontWeight:'400' , lineHeight:'24px', margin:'18px 0 30px', display:'inline-block' }}>
                        {recom.comment}
                      </Typography>
